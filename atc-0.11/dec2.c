...
char* usage =
"Usage: mdmc-dec2 [OPTION ...] PUB_KEY PRIV_KEY FILE\n"
"\n"
...
/* 		} */
	...
	if( !out_file )
	{
		if(  strlen(in_file) > 12 &&
				!strcmp(in_file + strlen(in_file) - 12, ".enc.enc.rec") )
			out_file = g_strndup(in_file, strlen(in_file) - 12);
		else if(  strlen(in_file) > 8 &&
...
main( int argc, char** argv )
{
	...
	read_cpabe_file(in_file, &rcp_buf, &file_len, &aes_buf);
